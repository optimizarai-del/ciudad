...
import {
  Flame, Thermometer, Snowflake, LayoutGrid, X, Clock,
  AlertTriangle, MapPin, Send, Users, ChevronRight, History, ShieldAlert, Gauge
} from 'lucide-react'
import Layout from '../../components/Layout/Layout'
import { useRole } from '../../context/RoleContext'
import api from '../../utils/api'

// ── Metadatos de etapas (10, doc §3.4) ──
export const ETAPAS = [
...
const TEMP = {
  caliente: { label: 'Caliente', icon: Flame, dot: 'bg-red-500', bg: 'bg-red-50 dark:bg-red-950/30', ring: 'ring-red-300' },
  tibio: { label: 'Tibio', icon: Thermometer, dot: 'bg-amber-500', bg: 'bg-amber-50 dark:bg-amber-950/30', ring: 'ring-amber-300' },
  frio: { label: 'Frío', icon: Snowflake, dot: 'bg-blue-500', bg: 'bg-blue-50 dark:bg-blue-950/30', ring: 'ring-blue-300' },
}
...
export default function Pipeline() {
  ...
  const [vista, setVista] = useState('temperatura')   // temperatura | etapas
  const [data, setData] = useState({ clientes: [], etapas: [] })
  const [consultas, setConsultas] = useState([])
  const [loading, setLoading] = useState(true)
  const [sel, setSel] = useState(null)                // cliente abierto en drawer

  const cargar = async () => {
    setLoading(true)
    try {
      const { data } = await api.get('/api/ventas-crm/pipeline')
      setData(data)
    } catch { setData({ clientes: [], etapas: [] }) } finally { setLoading(false) }
    if (esAdmin) {
  ...
  const resolver = async (qid, accion) => {
    try {
      await api.post(`/api/ventas-crm/pipeline/consultas/${qid}/resolver`, { accion })
      cargar()
    } catch {}
  }
  ...
  return (
    <Layout>
      <div className="max-w-[1400px] mx-auto animate-fade-in">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
            <div>
              <h1 className="hero-title text-3xl sm:text-4xl mb-1 flex items-center gap-2">
                <Users className="text-[#B8893A]" size={28} /> Pipeline
              </h1>
              <p className="hero-sub">Cada cliente, su temperatura y qué hacer con él hoy.</p>
            </div>
        ...
        {/* Consultas de degradación pendientes (líder) — doc §7.1 */}
        ...
        {loading ? (
          <div className="card text-center py-24 text-muted">Cargando pipeline…</div>
        ) : data.clientes.length === 0 ? (
          <div className="card text-center py-24 text-muted">Todavía no hay clientes en el pipeline.</div>
        ) : vista === 'temperatura' ? (
          <PanelTemperatura clientes={data.clientes} onOpen={setSel} />
        ) : (
          <Kanban porEtapa={porEtapa} onOpen={setSel} />
        )}
      </div>
...
// ── Panel de Temperatura (vista principal por urgencia, doc §3.8) ──
function PanelTemperatura({ clientes, onOpen }) {
  return (
    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
      {clientes.map(c => <CardLead key={c.id} c={c} onOpen={onOpen} grande />)}
    </div>
  )
}

// ── Kanban por etapa (vista secundaria) ──
function Kanban({ porEtapa, onOpen }) {
  return (
    <div className="flex gap-3 overflow-x-auto pb-3">
      {ETAPAS.map(([k, label]) => (
        <div key={k} className="min-w-[260px] w-[260px] shrink-0">
          <div className={`flex items-center justify-between px-2 py-1.5 mb-2 rounded-lg text-[12px] font-semibold ${PAUSA.includes(k) ? 'bg-neutral-200 dark:bg-neutral-800 text-muted' : 'bg-[#B8893A]/10 text-[#B8893A]'}`}>
            <span className="truncate">{label}</span>
            <span className="chip-muted text-[10px]">{porEtapa[k]?.length || 0}</span>
          </div>
          <div className="space-y-2">
            {(porEtapa[k] || []).map(c => <CardLead key={c.id} c={c} onOpen={onOpen} />)}
          </div>
        </div>
      ))}
    </div>
...
function CardLead({ c, onOpen, grande }) {
  const t = TEMP[c.temperatura] || TEMP.tibio
  const Icon = t.icon
  return (
    <button onClick={() => onOpen(c)}
      className={`w-full text-left rounded-xl border border-border p-3 transition hover:shadow-md ${t.bg}`}>
      ...
      <div className="mt-2 pt-2 border-t border-border/60">
        ...
        {c.dias_sin_contacto != null && (
          <span className="text-[10px] text-muted block mt-0.5">{c.dias_sin_contacto}d sin contacto</span>
        )}
      </div>
    </button>
  )
...
// ── Drawer de ficha (perfil + interacción + etapa + timeline) ──
function FichaDrawer({ cliente, onClose, onChange }) {
  ...
  const [nuevaEtapa, setNuevaEtapa] = useState(cliente.etapa)
  ...
  useEffect(() => {
    api.get(`/api/ventas-crm/clientes/${cliente.id}/eventos`).then(r => setEventos(r.data || [])).catch(() => {})
  }, [cliente.id])
  ...
  const registrarAccion = async () => {
    if (!accion.proxima_accion_contexto.trim() || !accion.proxima_accion_fecha) {
      setMsg('La próxima acción es obligatoria (fecha + contexto).'); return
    }
    setBusy(true); setMsg('')
    try {
      await api.post(`/api/ventas-crm/clientes/${cliente.id}/interaccion`, {
        texto: accion.texto || '(sin nota)', origen: 'web',
      ...
      onChange()
      api.get(`/api/ventas-crm/clientes/${cliente.id}/eventos`).then(r => setEventos(r.data || []))
  ...
  const cambiarEtapa = async () => {
    ...
    try {
      ...
      setMsg('✓ Etapa actualizada'); onChange()
      api.get(`/api/ventas-crm/clientes/${cliente.id}/eventos`).then(r => setEventos(r.data || []))
    } catch (e) { setMsg(e?.response?.data?.detail || 'Error') } finally { setBusy(false) }
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end" onClick={onClose}>
      ...
      <div className="relative w-full max-w-md bg-white dark:bg-[#0A0A0A] h-full overflow-y-auto shadow-2xl animate-slide-in-right"
        ...
        <div className="p-4 space-y-3">
          {tab === 'accion' && (
            <>
              ...
              <button className="btn-primary w-full" disabled={busy} onClick={registrarAccion}>
                <Send size={14} /> Registrar interacción
              </button>
            </>
          )}

...
import { Plus, FileText, Pencil, Trash2, X, Calendar, TrendingUp } from 'lucide-react'
      ...
      <div className="max-w-6xl mx-auto animate-fade-in">
        ...
        {filtered.length === 0 ? (
          <div className="card text-center py-24">
            <FileText size={40} className="mx-auto text-muted/30 mb-4" />
            <p className="text-muted text-[15px] mb-4">No hay contratos en esta categoría.</p>
            <button className="btn-primary" onClick={() => { setEditing(null); setOpen(true) }}>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {filtered.map(c => (
              <div key={c.id} className="card p-6 card-hover">
                <div className="flex items-start justify-between gap-3 mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-muted">{TIPO_LABEL[c.tipo] || c.tipo}</p>
                    </div>
                  </div>
                ...
                <div className="flex items-center gap-3 mt-4 border-t border-border pt-4 text-[12px] text-muted">
                  <Calendar size={12} />
                  <span>
...
function Row({ label, value, bold }) {
  return (
    <div className="flex items-center justify-between gap-2">
      ...
      <span className={`${bold ? 'font-semibold text-primary' : 'text-primary'} truncate max-w-[55%] text-right`}>{value}</span>
    </div>
  )
}
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 grid place-items-center p-4 overflow-auto"
      ...
      <div className="card p-8 w-full max-w-xl shadow-lift animate-scale-in my-6"
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <p className="text-[11px] uppercase tracking-[0.12em] text-muted font-semibold">Ajuste de precio</p>
